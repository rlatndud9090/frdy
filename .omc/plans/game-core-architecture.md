...
- 턴 순서: 용사 → (마왕 개입) → 적 → 반복
- 페이즈: HERO_TURN, DEMON_LORD_TURN, ENEMY_TURN
- next_turn(): 다음 턴으로 진행
- **수락 기준**: 턴 순서가 올바르게 순환
...
**TODO 7.4**: ManaManager 구현
- 현재 마력, 최대 마력
- spend(amount): 비용 차감 (부족 시 실패)
- restore(amount), reset_turn(): 턴 시작 시 복구
- **수락 기준**: 마력 소모/회복 동작. 부족 시 카드 사용 불가
