...
- 아래 키워드가 사용자 요청에 포함되면 `codex-pr-review-loop` 스킬을 우선 적용한다.
  - `PR 코멘트 확인`, `리뷰 코멘트 반영`, `Codex 리뷰 처리`, `re-review`, `재리뷰`, `@codex review`
- 위 스킬이 트리거되면 다음 단계를 **한 세트**로 처리한다.
  1. PR 리뷰/스레드 코멘트 수집
  2. 타당성 판정(accept/reject/needs-clarification)
  3. 반영 가능한 수정 적용 및 검증
  4. 커밋/푸시
  5. 리뷰 스레드 답글/resolve
  6. PR에 `@codex review` 재요청 코멘트 작성
